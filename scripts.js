...
function solve(){
    if(eval(coefA.value) !== undefined && eval(coefB.value) !== undefined && eval(coefC.value) !== undefined){
        ...
        if (discriminant<0){
            result.innerHTML = "Imaginary Roots.";
        }
        else if (discriminant===0){
            let root1 = (-b+Math.sqrt(discriminant))/(2*a);
            let root2 = (-b-Math.sqrt(discriminant))/(2*a);
            result.innerHTML = "Real And Equal Roots.<br>";
            ...
            let root1 = (-b+Math.sqrt(discriminant))/(2*a);
            let root2 = (-b-Math.sqrt(discriminant))/(2*a);
            result.innerHTML = "Real And Distinct Root.<br>";
            result.innerHTML += "x<sub>1</sub>: " + root1 + "<br>";
            result.innerHTML += "x<sub>2</sub>: " + root2 + "<br>";
        }
    }
    else{
        result.innerHTML = "Values of all the coefficients must be provided";
    }
}
